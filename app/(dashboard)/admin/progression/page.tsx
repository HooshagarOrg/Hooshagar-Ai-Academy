    ...
    >
      ...
      {activeTab === 'manage' && (
        <div className="space-y-5">
          {/* نتیجه ارتقاء */}
          {result && (
            <div className="lux-dash-card rounded-2xl border border-emerald-500/25 bg-emerald-500/10 p-5">
              ...
              <div className="grid grid-cols-3 gap-4">
                {[
                  ...
                  { label: 'ارتقاء یافته', value: result.promoted, color: 'text-emerald-400' },
                  { label: 'ناموفق', value: result.failed, color: 'text-red-400' },
                ].map(s => (
                  <div key={s.label} className="lux-dash-stat rounded-xl p-3 text-center">
          ...
          {summary && (
            <div className="grid grid-cols-3 gap-4">
              {[
                ...
                { label: 'واجد شرایط', value: summary.eligible, color: 'text-emerald-400', bg: 'bg-emerald-500/10 border border-emerald-500/20' },
                { label: 'غیرواجد', value: summary.not_eligible, color: 'text-red-400', bg: 'bg-red-500/10 border border-red-500/20' },
              ].map(s => (
          ...
          {/* تنظیمات ارتقاء */}
          <div className="lux-dash-card rounded-2xl p-5">
                ...
                {filtered.map(student => (
                  <div
                    ...
                    className={cn(
                      ...
                      selectedIds.has(student.id) && 'bg-emerald-500/10'
                    )}
                  >
                      <input
                      type="checkbox"
                    ...
                    <div className="flex items-center gap-3 flex-shrink-0">
                      ...
                      {student.eligible ? (
                        <Badge className="bg-emerald-500/15 text-emerald-300 text-xs gap-1">
                          <CheckCircle2 className="w-3 h-3" />
                          واجد شرایط
                        </Badge>
                      ) : (
                        <Badge className="bg-red-500/15 text-red-300 text-xs gap-1">
        ...
        {history.map(h => (
          <div key={h.id} className="flex items-center gap-3 px-5 py-3 hover:bg-[var(--lux-surface)]">
            ...
            <div className="flex items-center gap-2">
              ...
              <Badge className={cn(
                'text-xs',
                h.status === 'completed' ? 'bg-emerald-500/15 text-emerald-300' : 'bg-red-500/15 text-red-300'
              )}>
