...
// ============================================================
// مدیریت سهمیه platform_admin در قرعه‌کشی
//
// GET  ?period_id=...           — لیست دانش‌آموزان + وضعیت سهمیه
// POST { action: 'assign' }    — تخصیص سهمیه به یک دانش‌آموز
// POST { action: 'revoke' }    — لغو تخصیص (انصراف → دستی جایگزین)
// POST { action: 'set_quota' } — تنظیم تعداد سهمیه هر کلاس
// ============================================================
...
async function requirePlatformAdmin(supabase: Awaited<ReturnType<typeof createClient>>) {
  ...
  return p?.role === 'platform_admin' ? user : null
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    // دانش‌آموزان بدون تخصیص (not_assigned) — کاندید سهمیه
...
// ── POST: عملیات تخصیص / لغو / تنظیم سهمیه ──────────────────
