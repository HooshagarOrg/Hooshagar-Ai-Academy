  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: `
            radial-gradient(ellipse 60% 55% at 85% 0%, rgba(var(--role-accent-r), 0.12), transparent 60%),
            radial-gradient(ellipse 50% 45% at 10% 100%, rgba(var(--role-accent-r), 0.07), transparent 55%)
          `,
        }}
        aria-hidden
      />
      ...
      <div className="relative z-10 px-6 py-6 sm:py-7 flex flex-col sm:flex-row sm:items-center gap-4">
        <div className="flex-1 min-w-0">
          {meta && (
            <p className="text-xs text-white/35 mb-1 tracking-wider font-mono">{meta}</p>
          )}
          <h1 className="text-2xl sm:text-3xl font-black text-white leading-tight">{title}</h1>
          {description && (
            <p className="text-sm text-white/45 mt-1.5 leading-relaxed">{description}</p>
          )}
        </div>
        {actions && (
