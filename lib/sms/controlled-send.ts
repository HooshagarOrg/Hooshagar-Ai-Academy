...
type LooseQuery = {
  ...
  filter: (column: string, operator: string, value: string) => LooseQuery
  maybeSingle: () => Promise<{
    data: { daily_sms_limit?: number } | null
...
function smsDb(): LooseDb {
  return createServiceClient() as unknown as LooseDb
}
