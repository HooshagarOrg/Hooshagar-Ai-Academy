/**
 * بینش زنده — HTML سرور + انیمیشن CSS. بدون React client.
 */
...
function StaticBarChart(): JSX.Element {
  return (
    <div className="lp-glass h-full p-5">
...
function StaticLineChart(): JSX.Element {
  const points = WEEKLY_POINTS
  const w = 280
  const h = 120
  const pad = 8
  const max = 100
...
function StaticRingChart(): JSX.Element {
  const r = 42
  const c = 2 * Math.PI * r
  let offset = 0
  const segs = RING_SEGMENTS.map((s) => {
      ...
      <div className="flex items-center gap-4">
        <div className="relative shrink-0">
          ...
          <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
            <span className="text-lg font-black tabular-nums text-[var(--lux-text)]">100</span>
...
export function LiveInsightsStatic(): JSX.Element {
  return (
