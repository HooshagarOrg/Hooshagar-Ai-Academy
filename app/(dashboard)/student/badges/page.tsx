...
export default function StudentBadgesPage() {
  ...
  return (
    <div dir="rtl">
      <LuxPageHeader title="گالری نشان‌ها" subtitle="دستاوردهای یادگیری و گیمیفیکیشن" />
      {loading ? (
        <div className="flex justify-center py-16"><Loader2 className="h-8 w-8 animate-spin text-[var(--lux-gold)]" /></div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {badges.map((b) => (
            <LuxCard key={b.id} className={!b.earned ? 'opacity-50' : ''}>
      ...
      {badges.every((b) => !b.earned) && !loading && (
