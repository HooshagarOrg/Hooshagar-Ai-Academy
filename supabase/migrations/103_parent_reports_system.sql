-- ============================================
-- Parent Reports System Migration
-- ============================================
-- Created: 2025-12-31
...
CREATE INDEX IF NOT EXISTS idx_homework_due_date ON homework_submissions(due_date);
...
CREATE INDEX IF NOT EXISTS idx_grades_subject ON student_grades(subject);
CREATE INDEX IF NOT EXISTS idx_grades_date ON student_grades(exam_date);

CREATE INDEX IF NOT EXISTS idx_behavior_student ON student_behavior(student_id);
CREATE INDEX IF NOT EXISTS idx_behavior_date ON student_behavior(date);
...
CREATE OR REPLACE FUNCTION calculate_student_stats(
  p_student_id UUID,
...
BEGIN
  ...
  WHERE student_id = p_student_id
    AND date BETWEEN p_period_start::DATE AND p_period_end::DATE;
  
  -- درصد انجام تکالیف
  SELECT COALESCE(
    (COUNT(*) FILTER (WHERE status IN ('submitted', 'graded')) * 100.0 / NULLIF(COUNT(*), 0)),
    0
  )
...
CREATE OR REPLACE FUNCTION mark_report_viewed(p_report_id UUID, p_parent_id UUID)
RETURNS BOOLEAN AS $$
BEGIN
  UPDATE parent_reports
